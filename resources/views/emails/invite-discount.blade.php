...
<div style="max-width:650px; margin:20px auto; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif">
   <div style="border-radius: 8px 8px 0 0; background-color: #2ba2da; padding:15px ">
    ...
    This is the invitation request from squeedr. 
    <?php
    if($discount > 0)
    {
    ?>
    You are eligible for '<?=$discount;?>%' discount for availing services with <?=$service_provider_name;?>
    <?php
      }
      else
      {
      ?>
      You are eligible discount for availing services with <?=$service_provider_name;?>
      <?php
      }
    ?>
    Please go through our squeedr page to check our latest services.
    <br>
    <?=url('business-provider').'/'.$service_provider_username;?>
